...
> **BannerHub patch (v3.7.1 stable, originally v3.7.1-pre1):** This call is **stubbed** in BannerHub. `SettingUpgradeHolder$onBind$1$1.invokeSuspend` has the `AppUpgradeRepo.getAppUpgradeApk()` suspend invocation replaced with `const/4 p1, 0x0`, forcing the result to `null`. Existing `if-eqz` checks in the same method then take the "no update" branch unconditionally (row reads "Already the latest version", tap toasts the same). No probe is sent to GameHub's upgrade endpoint, regardless of which API the user has selected. This avoids the footgun where upstream's 6.0.1 / versionCode 110 response would otherwise prompt 5.3.5-base BannerHub variants (versionCode 78) to download stock GameHub. Shipped to all 9 variants in **v3.7.1 (2026-05-10)** as a hotfix on top of v3.7.0. See `PROGRESS_LOG.md` § v3.7.1.

> **BannerHub patch (v3.7.2 stable, 2026-05-12):** v3.7.1 only closed the *manual* probe (Settings → About → Check Update row). The same `AppUpgradeRepo.b()` singleton is also called *automatically* on every cold start through `ApkUpdateUtils.c(activity)` → `ApkUpdateUtils$checkUpdate$1$1.invokeSuspend`, which then fed an auto-popping dialog offering the user to download stock GameHub. **v3.7.2** stubs that second call site with the same anchor-patch shape — `smali_classes6/com/xj/landscape/launcher/utils/ApkUpdateUtils$checkUpdate$1$1.smali` has the `sget-object AppUpgradeRepo` … `invoke-virtual ->b(I, Continuation)` … `move-result-object p1` block replaced with `const/4 p1, 0x0`. `ApkUpdateUtils.e()` already null-checks (`if-eqz p1, :cond_2`) and returns without showing the dialog. Both call sites are now silent. Shipped to all 9 variants in **v3.7.2 (2026-05-12)** as a hotfix on top of v3.7.1. See `PROGRESS_LOG.md` § v3.7.2.
